...
pub mod running {
    ...
    pub trait Running {
        ...
        fn num_splits(&self, race: &impl Race) -> u64;
        ...
        fn negative_splits(&self, race: &impl Race, degree: Duration) -> Vec<Duration> {
            ...
            let num_splits = self.num_splits(race);
        ...
        fn splits_with_pace(&self, race: &impl Race, pace: Duration) -> Vec<Duration> {
            ...
            for _n in 0..self.num_splits(race) {
                splits.push(pace);
            }
    ...
    impl Running for MetricRunning {
        ...
        fn num_splits(&self, race: &impl Race) -> u64 {
            race.distance() / MetricRace::SPLIT_DISTANCE + if (race.distance() % MetricRace::SPLIT_DISTANCE) > 0 { 1 } else { 0 }
        }
    ...
    impl Running for ImperialRunning {
        ...
        fn num_splits(&self, race: &impl Race) -> u64 {
            race.distance() / ImperialRace::SPLIT_DISTANCE + if (race.distance() % ImperialRace::SPLIT_DISTANCE) > 0 { 1 } else { 0 }
        }
